...
const ctx=vm.createContext({mapCandidateToken:0,mapInspectorView:null,mapPotential:{graph:{nodes:[{id:"hide",name:"Hide",kind:"item",missing:40}]}},$:id=>id==='mapOptions'?host:id==='mapFindSources'?{after:h=>{placed=h===host;}}:null,state:{areas:['Forest','Cave'],map_sources:{mushroom:['Forest']}},mapGraph:{nodes:[{id:'mushroom',kind:'item',missing:100},{id:'hide',name:'Hide',kind:'item',missing:0}]},mapPlan:{areas:[]},mapPreview:{preview_plan:{areas:[{location_id:'Forest'}],optimal_total_explores:1000}},catalog:{locations:[{id:'Forest',name:'Forest'},{id:'Cave',name:'Cave'}]},api:async path=>({sources:path.endsWith('mushroom')?[source('Forest',1),source('Cave',2)]:[source('Forest',.5)]}),explorationYieldMultiplier:()=>1,mapCount:q=>String(Math.floor(q)),fmt:String,esc:String,itemName:(_,name)=>name});
...
(async()=>{
 ...
 assert.match(cards[1][2],/Explore more here/);assert.match(cards[1][2],/\+100 explores &middot; \+10.0%/);assert.match(cards[1][2],/Hide · Covers this shortage/);assert.match(host.innerHTML,/Remove Forest source/);
