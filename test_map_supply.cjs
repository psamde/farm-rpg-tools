...
vm.runInContext(`mapRender=()=>{};mapSelect=()=>{};mapFocus=()=>{};mapPlan={areas:[{location_id:'cave'}]};mapGraph={missing:{wood:12}};mapDrafts=[{item_id:'canteen',cap:100,user_cap:true,consumer_mode:'available',available_only:true,allow_exploration:false}];`,ctx);
(async()=>{
 const area={dataset:{mapArea:'forest',mapSourceItem:'wood'}};
 await el('mapInspector').onclick({target:{closest:s=>s==='[data-map-area]'?area:null}});
 const initial=calls.find(c=>c.path==='/api/plan').payload;
 ...
 assert.equal(initial.map_planning,true);assert.deepEqual(Array.from(initial.map_sources.wood),['forest']);
