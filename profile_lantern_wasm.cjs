...
`);const runs=[];for(const quantity of [90001]){quantityNow=quantity;payload.targets['837']=quantity;py.globals.set('payload_json',JSON.stringify(payload));timings=[];const started=performance.now();const result=py.runPython('json.dumps(compute(catalog,json.loads(payload_json)))');const parsed=JSON.parse(result);const expected={90000:38775741,90001:38776399}[payload.targets['837']];if(expected!==undefined && parsed.plans[0].optimal_total_explores!==expected)throw Error('Unexpected explore total');for(const plan of parsed.plans){if(plan.targets[0].craft_quantity!==payload.targets['837'])throw Error('Target changed');for(const row of plan.item_balances){if(row.expected_final_inventory < -1e-6)throw Error('Negative inventory: '+row.name);}}const row={quantity,seconds:(performance.now()-started)/1000,solverSeconds:timings.reduce((sum,t)=>sum+t.ms,0)/1000,calls:timings.length,slowest:[...timings].sort((a,b)=>b.ms-a.ms).slice(0,3)};runs.push(row);console.log(JSON.stringify(row));fs.writeFileSync('../../work/lantern-profile.json',JSON.stringify(runs,null,2));}
