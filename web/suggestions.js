...
function suggestCrafts(items, stock, settings, material=null, limit=5, focusBound=false){
 ...
 for(const item of Object.values(items)){
  ...
  const bound=Math.min(100000000,Math.ceil(focusBound&&material?(stock[material]||0)/needs[material]:Object.entries(stock).reduce((sum,[id,q])=>sum+(needs[id]?q/needs[id]:0),0)));
  if(!bound)continue;
 ...
 const rank=rows=>rows.sort((a,b)=>b.score-a.score||items[a.id].name.localeCompare(items[b.id].name)).slice(0,limit);
