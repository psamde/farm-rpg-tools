...
function comparisonLabel(row){return row.comparison_status==='pending'?'Checking if first…':row.comparison_status==='unavailable'?'Comparison unavailable':row.lost_to_priority?`${fmt(row.if_first)} alone on this route`:'planned crafts';}
function secondaryRows(showResults=false){
 ...
 $('priorityStatus').textContent=!s?(result?'Allocating shared leftovers…':'Add inputs to start planning.'):s.priority_matters?`Option ${selected+1}: Balanced allocation: these crafts share ingredients. Move an item up to favor it, or use caps to adjust the mix.`:`Option ${selected+1}: These crafts can use the available ingredients without a significant tradeoff.`;
