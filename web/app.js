...
function refreshCosts(){ciderSettings();if(result&&!$('resultContent').classList.contains('stale')){detail();costPreview();}}
...
function updateTargetSavings(){
 document.querySelectorAll('[data-target-savings]').forEach(el=>{
  ...
  el.title='Estimated savings if you provide all of the remaining quantity from inventory. Uses the last completed comparison, including shared drops and inventory. Leftover crafts may change the final route; estimates are not additive.';
 });
}
function syncTargetInputs(id){
 const total=state.targets[id],provided=state.provided_targets?.[id]?.quantity||0;
 $('goal-'+id).value=total;$('target-'+id).max=total;$('provided-'+id).max=total;
 $('target-'+id).value=total-provided;$('provided-'+id).value=provided;
}
