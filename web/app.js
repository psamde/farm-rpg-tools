...
function provideMissingInventory(row,token){
 ...
 if(!items[row.item_id]||!Number.isSafeInteger(row.quantity)||row.quantity<=0||!Number.isFinite(quantity)||quantity>1e12){error('That amount exceeds the inventory limit. Adjust Starting inventory in Settings.');return;}
...
function targetEffortRows(){let personal={};try{personal=JSON.parse(localStorage.getItem('farm-tower-planner-v1')||'{}')||{};}catch{}return TargetCosts.rank(catalog,state,personal);}
function targets(){
 $('targetCount').textContent=Object.keys(state.targets).length;
 $('targets').innerHTML=targetEffortRows().map(({id,quantity:q,remaining,cost},index)=>{
  const supplied=state.provided_targets?.[id]?.quantity||0;
  return `<div class="targetrow ${remaining?'':'target-provided'}"><div class="name"><span class="target-rank" title="Highest effort first">${index+1}</span><div>${itemName(id,items[id].name)}${remaining?externalIngredientNote(id):''}<small class="target-effort" title="Tower Planner effort score: gathering actions, passive supply and configured market prices. Shared drops and starting stock are handled separately in the explore-savings estimate.">${remaining?(Number.isFinite(cost.points)?fmt(Math.floor(cost.points))+' effort points':'Effort unknown · missing source data'):'Provided from inventory'}${supplied&&remaining?' · '+fmt(supplied)+' already provided':''}</small></div></div><label class="targetquantity" for="target-${id}"><span>${items[id].craftable?'Craft quantity':'Collect quantity'}</span><input id="target-${id}" data-quantity="${id}" type="number" min="1" max="100000000" step="1" value="${q}" ${supplied?'disabled title="Undo the inventory supply to edit this quantity"':''}></label><div class="target-inventory">${remaining?`<button type="button" data-provide-target="${id}">Provide from inventory</button><small data-target-savings="${id}"></small>`:''}${supplied?`<span class="target-provided-note">${fmt(supplied)} set aside for this goal</span><button type="button" data-undo-target="${id}">Undo</button>`:''}</div><button class="iconbutton" type="button" data-remove="${id}" aria-label="Remove ${esc(items[id].name)}">×</button></div>`;
 }).join('');
 updateTargetSavings();
}
function updateTargetSavings(){
 document.querySelectorAll('[data-target-savings]').forEach(el=>{
  ...
  el.textContent=Number.isFinite(q)?(q>=0?`≈ ${fmt(Math.floor(q))} fewer explores${base>0?' · '+(100*q/base).toFixed(1)+'%':''}`:`≈ ${fmt(Math.ceil(-q))} more explores`):row?.unavailable||targetSavings?.unavailable?'Estimate unavailable':'Estimating explore savings…';
  el.title='Compared with the combined primary goals, including shared drops and inventory. Leftover crafts may change the final route. Estimates are not additive.';
 });
}
...
function changed(){targetSavings=null;targets();startingSupplyReport(null);state.secondary=state.secondary.filter(r=>!state.targets[r.item_id]);save();revision++;if(state.planner_mode!=='passive'&&!Object.keys(state.targets).length){clearTimeout(timer);resetEmptyPlan();return;}secondaryRows();$('resultContent').classList.add('stale');$('status').textContent='Targets or settings changed. Recalculating…';clearTimeout(timer);timer=setTimeout(calculate,350);}
...
$('targets').addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.provideTarget){provideTarget(b.dataset.provideTarget);return;}if(b.dataset.undoTarget){undoProvidedTarget(b.dataset.undoTarget);return;}if(b.dataset.remove){TargetCosts.undo(state,b.dataset.remove);delete state.targets[b.dataset.remove];inventoryRows();changed();}});
...
for(const [id,key] of [['ironDepot','iron_depot'],['runecube','runecube'],['cockatriceEtherSource','cockatrice_ether_source'],['cockatriceEtherSource','cockatrice_ether_source']])$(id).addEventListener('change',()=>{state[key]=$(id).checked;changed();});
...
function renderRoute(){if(!result)return;$('routePanel').hidden=false;$('routeTotal').textContent=fmt(result.plans[selected].optimal_total_explores)+' expected explores';$('routePanel').classList.remove('stale');$('craftworksSlots').value=state.craftworks_slots||10;$('routeOption').hidden=state.planner_mode!=='passive';$('inventorySize').value=state.inventory_size||10000;$('routeMethod').value=state.route_method||'AP';$('routeOption').textContent=state.planner_mode==='passive'?`Per ${state.production_interval===10?'10 minutes':'hour'} of production`:'';renderRouteFoods();
 ...
 $('routeStatus').textContent=r.problem?`Loop estimate — inventory verification incomplete: ${r.problem}`:r.parts.length?`Repeat this circuit ${fmt(r.rounds)} ${r.rounds===1?'time':'times'}. ${fmt(r.uses)} ${drink} total${r.method==='Cider'?` + ${fmt(Math.ceil(r.stamina))} stamina`:''} · ≈ ${fmt(r.extraExplores)} extra explores from repeatable whole-drink batches. Collected and crafted items checked against your ${fmt(r.capacity)} limit throughout the loop.`:state.planner_mode==='passive'?'No exploration needed. Craft from this production batch.':(!Object.keys(activeTargets()).length&&!p.crafts_in_dependency_order.length&&!p.secondary?.crafts_in_dependency_order?.length?'All primary targets are provided. No exploration or crafting needed.':'No exploration needed. Craft from starting inventory.');
...
function resetEmptyPlan(){startingSupplyReport(null);browserPlanner.cancel();result=null;selected=0;state.selected_plan=0;state.selected_plan_key="";save();lastSuccess=null;$('empty').hidden=false;$('resultContent').hidden=true;$('resultContent').classList.remove('stale');$('routePanel').hidden=true;$('status').textContent='Add a crafting or collection target to start.';$('compute').disabled=false;$('status').classList.remove('loading');$('implicitCrafts').innerHTML='';$('rawMaterials').innerHTML='';secondaryRows(false);}
