...
function craftworksSetup(plan,loop){
 ...
 return `<details id="craft-stop-group-${i}" class="craftworksset"><summary>${group.recipes.length?'Keep active':'Collect'}: ${esc(group.from)}${group.from===group.to?'':' → '+esc(group.to)}${group.recipes.length?' · '+group.recipes.length+' / '+loop.slots+' slots':' · craft later'}</summary><p class="hint">${group.recipes.length?'These recipes keep ingredient stacks within capacity or supply later crafts. Required recipes come first; finished items stay above their ingredients.':'No active Craftworks needed here. Keep the collected ingredients for the later setups.'}</p>${group.recipes.length?`<div class="craftworkstable"><table><thead><tr><th>Recipe</th><th>Crafts per loop ≈</th></tr></thead><tbody>${craftworksDisplayOrder(group.recipes,loop.requiredRecipes).map(id=>`<tr><td>${itemName(id,items[id]?.name||id)}</td><td>${(group.amounts?.[id]||0)<1?'&lt;1':fmt(group.amounts[id])}</td></tr>`).join('')}</tbody></table></div>`:''}</details>`+(after?checkpoint(after):'');
 }).join('')+(finish?checkpoint(finish):'');
 return `<div class="sectiontitle"><h3>Craftworks by location</h3></div><p class="hint">Craft later when the ingredients fit. Early setups protect inventory space or supply a later step; spare slots alone are not a reason to craft early.</p>${sections||'<p class="hint">No crafting needed.</p>'}<p class="hint">Amounts are averages per loop; final cleanup is separate. Keep the listed quantities for your goals. These are planned quantities, not automatic Craftworks limits—stop completed recipes. Tick and swap times are not modeled.</p>`;
}
