...
const defaults={selected_plan:0,selected_plan_key:"",performance:{cache_primary:false,reuse_models:false,fractional_start:false},planner_mode:'goals',production_interval:60,passive_inventory:'{}',craftworks_slots:10,inventory_size:10000,route_method:'AP',route_foods:{},secondary:[],theme:'dark',resource_saver:0,wanderer:0,lemon_squeezer:false,cinnamon:false,cider_rolls:{},effectiveness_upgrades:{},sprint_shoes:0,targets:{'167':1000},iron_depot:true,runecube:false,mode:'best',max_areas:3,areas:null,inventory:'{}'};
...
function save(){try{localStorage.setItem('farm-workshop-v1',JSON.stringify(state));}catch{}}
...
for(const [id,key] of [['cachePrimary','cache_primary'],['reuseModels','reuse_models'],['fractionalStart','fractional_start']])$(id).onchange=()=>{state.performance={...defaults.performance,...state.performance,[key]:$(id).checked};changed();};
$('benchmarkAgain').onclick=()=>{clearTimeout(timer);calculate();};
...
$('planDetail').addEventListener('click',e=>{const b=e.target.closest('[data-consumer]');if(b)showConsumers(b.dataset.consumer);});

let consumerRevision=0;

let suggestionMaterial=null;
async function showConsumers(id){suggestionMaterial=id;const token=++consumerRevision;$('consumerTitle').innerHTML='Craft with '+itemName(id,items[id].name);$('consumerSearch').value='';$('consumerList').textContent='Finding recipes…';$('consumerDialog').showModal();try{const rows=await api('/api/consumers/'+id);if(token!==consumerRevision)return;consumers=rows;consumerList();}catch(e){$('consumerList').textContent=e.message;}}
...
function addSecondary(id){if(state.targets[id])throw Error('This item is already a primary target.');if(!items[id]?.craftable)throw Error('Choose a craftable item.');if(state.secondary.some(r=>r.item_id===id))return;if(state.secondary.length>=20)throw Error('Use at most 20 leftover targets.');state.secondary.push({item_id:id,allow_exploration:false,cap:null});changed();}
...
function secondaryRows(showResults=false){
 ...
 return `<div class="secondaryrow" data-leftover-id="${r.item_id}"><div class="prioritycontrols"><button type="button" class="draghandle" data-drag-leftover="${r.item_id}" aria-label="Drag to reorder ${esc(items[r.item_id].name)}" title="Drag to reorder">⠿</button><button type="button" data-secondary-up="${i}" aria-label="Prioritize ${esc(items[r.item_id].name)}" ${i===0?'disabled':''}>↑</button><button type="button" data-secondary-down="${i}" aria-label="Lower priority of ${esc(items[r.item_id].name)}" ${i===state.secondary.length-1?'disabled':''}>↓</button></div><div class="secondaryname"><span class="rank">${i+1}</span>${itemName(r.item_id,items[r.item_id].name)}${bottleneckLabel(r,row,result?.plans[selected])}</div><label class="secondarycap">Cap<input type="number" data-secondary-cap="${i}" aria-label="Cap for ${esc(items[r.item_id].name)}" min="0" max="100000000" step="1" placeholder="Max" value="${r.cap??''}"></label><label class="check secondaryexplore"><input type="checkbox" data-secondary-explore="${i}" ${r.allow_exploration?'checked':''}> Allow extra exploring</label><div class="secondaryamount"><strong>${row?fmt(row.crafts):'—'}</strong><small>${row?comparisonLabel(row):result?'calculating':'not planned'}</small></div><button class="iconbutton" type="button" data-secondary-remove="${i}" aria-label="Remove leftover target ${esc(items[r.item_id].name)}">×</button>

 ${bottleneckLabel(r,row,result?.plans[selected],true)}${externalIngredientNote(r.item_id)}
 ${row?.crafts===0&&!leftoverBottleneck(r,row,result?.plans[selected])?`<div class="secondaryextras">${row.no_pool_inputs?`<p class="hint">${state.planner_mode==='passive'?'Add one of this recipe’s ingredients to your production amounts to get started.':'None of your leftovers can go into this recipe yet. Add it as a primary target to plan for it directly.'}</p>`:row.missing_for_next_craft?.length?`<p class="missinginputs">Cannot craft yet. Missing: ${row.missing_for_next_craft.map(m=>itemName(m.item_id,m.name)).join(' · ')}</p>`:''}</div>`:''}</div>`;

 }).join('');

 $('priorityStatus').textContent=!s?(result?'Allocating shared leftovers…':'Add inputs to start planning.'):s.priority_matters?`Option ${selected+1}: priority matters. These crafts compete for the same leftovers; move an item up to favor it.`:`Option ${selected+1}: no priority conflict. These crafts can share the leftovers without taking resources from one another.`;
...
function starterPlan(){
 return {...withSettingDefaults(),targets:{'126':1000},
  secondary:[{item_id:'83',allow_exploration:true,cap:null},{item_id:'81',allow_exploration:false,cap:null}],
...
function validatePlanSettings(input){
 ...
 for(const r of out.secondary){if(!object(r)||!items[r.item_id]?.craftable||seen.has(r.item_id)||typeof r.allow_exploration!=='boolean'||r.cap!=null&&(!Number.isInteger(r.cap)||r.cap<0||r.cap>100000000))fail('Invalid leftover target');seen.add(r.item_id);}
 ...
 if(!object(out.performance)||Object.entries(out.performance).some(([k,v])=>!Object.hasOwn(defaults.performance,k)||typeof v!=='boolean'))fail('Invalid performance options');
...
for(const host of [$('suggestionResults')])host.addEventListener('click',e=>{
 ...
 if(!state.secondary.some(g=>g.item_id===button.dataset.suggestion))state.secondary.push({item_id:button.dataset.suggestion,allow_exploration:button.dataset.assist==='true',cap:null});
 $('suggestionsDialog').close();$('consumerDialog').close();changed();}catch(err){error(err.message);}
});

// Keep the solver idle while dragging; commit the new priority only on drop.
