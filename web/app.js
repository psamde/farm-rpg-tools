...
async function calculate(){state.secondary=state.secondary.filter(r=>!state.targets[r.item_id]);const token=++revision;error('');if(state.planner_mode!=='passive'&&!Object.keys(state.targets).length){resetEmptyPlan();return;}let payload;try{payload={planner_mode:state.planner_mode,production_interval:state.production_interval,targets:state.planner_mode==='passive'?{}:{...state.targets},secondary:state.secondary.map(r=>({...r,allow_exploration:r.allow_exploration&&!!r.exploration_item_id})),areas:[...state.areas],iron_depot:state.iron_depot,runecube:state.runecube,resource_saver:state.resource_saver,combinations_mode:state.mode==='combinations',max_areas:state.max_areas,inventory:inventoryData()};}catch{error('Starting inventory must be valid JSON, for example {"Wood": 100}.');$('status').textContent='Check starting inventory.';$('compute').disabled=false;$('status').classList.remove('loading');return;}
...
function secondaryRows(showResults=false){
 ...
 return `<div class="secondaryrow" data-leftover-id="${r.item_id}"><div class="prioritycontrols"><button type="button" class="draghandle" data-drag-leftover="${r.item_id}" aria-label="Drag to reorder ${esc(items[r.item_id].name)}" title="Drag to reorder">⠿</button><button type="button" data-secondary-up="${i}" aria-label="Prioritize ${esc(items[r.item_id].name)}" ${i===0?'disabled':''}>↑</button><button type="button" data-secondary-down="${i}" aria-label="Lower priority of ${esc(items[r.item_id].name)}" ${i===state.secondary.length-1?'disabled':''}>↓</button></div><div class="secondaryname"><span class="rank">${i+1}</span>${itemName(r.item_id,items[r.item_id].name)}${bottleneckLabel(r,row,result?.plans[selected])}</div><label class="secondarycap">Cap<input type="number" data-secondary-cap="${i}" aria-label="Cap for ${esc(items[r.item_id].name)}" min="0" max="100000000" step="1" placeholder="Max" value="${r.cap??''}"></label><div class="secondaryexplore"><label class="check"><input type="checkbox" data-secondary-explore="${i}" ${r.allow_exploration?'checked':''}> Explore for missing ingredients</label>${r.allow_exploration?`<label class="anchorchoice">Use up <select data-secondary-anchor="${i}" aria-label="Ingredient to use up for ${esc(items[r.item_id].name)}"><option value="">Choose an ingredient…</option>${[true,false].map(direct=>`<optgroup label="${direct?'Direct ingredients':'Ingredients further down the recipe'}">${explorationIngredients(r.item_id).filter(id=>Object.hasOwn(items[r.item_id].direct_ingredients,id)===direct).filter(id=>!(state.iron_depot&&['22','38'].includes(id))).map(id=>`<option value="${id}" ${id===r.exploration_item_id?'selected':''}>${esc(items[id].name)}</option>`).join('')}</optgroup>`).join('')}</select></label>`:''}</div><div class="secondaryamount"><strong>${row?fmt(row.crafts):'—'}</strong><small>${row?comparisonLabel(row):result?'calculating':'not planned'}</small></div><button class="iconbutton" type="button" data-secondary-remove="${i}" aria-label="Remove leftover target ${esc(items[r.item_id].name)}">×</button>

 ${r.allow_exploration?`<div class="secondaryextras anchorhint"><p class="hint">${r.exploration_item_id?`The goal is set by ${itemName(r.exploration_item_id,items[r.exploration_item_id].name)} available from your original leftovers. Explore for the rest. New drops can supply extra crafts without extending the route.`:'Choose an ingredient above to enable extra exploring. Until then, this craft uses available leftovers only.'}</p></div>`:''}${bottleneckLabel(r,row,result?.plans[selected],true)}${externalIngredientNote(r.item_id)}
