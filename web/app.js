...
const defaults={map_planning:false,map_unused_mode:"top",map_unused_count:6,map_expanded_areas:[],map_sources:{},map_node_usage:{},map_use_void:[],map_hidden:[],map_voided:[],tower_level:0,map_positions:{},guided_skipped:[],quest_selection:[],selected_plan:0,selected_plan_key:"",planner_mode:'goals',production_interval:60,passive_inventory:'{}',craftworks_slots:10,inventory_size:10000,route_method:'AP',route_foods:{},secondary:[],automatic_areas:null,theme:'dark',resource_saver:0,wanderer:0,lemon_squeezer:false,cinnamon:false,cider_rolls:{},effectiveness_upgrades:{},sprint_shoes:0,targets:{'167':1000},iron_depot:true,runecube:false,cockatrice_ether_source:false,cockatrice_ether_source:false,mode:'best',max_areas:15,areas:null,inventory:'{}'};
...
function startingSupplyReport(plan){
 ...
 $('autoInventory').innerHTML=auto.length?`<h3>Automatically added for primary targets</h3><p class="hint">Bring these supplies from outside exploration. Amounts update with the selected plan; they aren’t extra items you already own.</p><div class="starting-supplies">${auto.map(b=>`<div>${itemName(b.item_id,b.name)}<strong>${fmt(b.auto_starting_inventory)}</strong></div>`).join('')}</div>`:'';
}
...
$('confirmClearPlan').onclick=()=>{
 ...
 state.targets={};state.secondary=[];state.map_planning=false;state.automatic_areas=null;state.guided_skipped=[];state.map_voided=[];state.map_use_void=[];state.map_sources={};state.map_node_usage={};state.map_hidden=[];state.map_expanded_areas=[];state.map_unused_mode='top';state.quest_selection=[];state.inventory='{}';state.passive_inventory='{}';
