...
function secondaryRows(showResults=false){
 ...
 ${r.allow_exploration?`<div class="secondaryextras anchorhint"><p class="hint">${r.exploration_item_id?`${row?.exploration_ingredient&&!items[r.exploration_item_id].craftable?`Available at this step: ${fmt(Math.floor(row.exploration_ingredient.available))} `:"Use "}${itemName(r.exploration_item_id,items[r.exploration_item_id].name)} from the shared pool.${row?.exploration_ingredient?` Extra exploring targets ${fmt(row.exploration_ingredient.planned_crafts)} crafts.`:" This step freezes its goal before exploring."} Later drops can supply extra crafts without growing that target.`:'Choose an ingredient above to enable extra exploring. Until then, this craft uses available leftovers only.'}</p></div>`:''}${bottleneckLabel(r,row,result?.plans[selected],true)}${externalIngredientNote(r.item_id)}
