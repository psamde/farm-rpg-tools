...
function mapQueuePreview(){clearTimeout(mapPreviewTimer);if(mapChangeCount()&&!mapBusy()){$('mapNodes').inert=true;$('mapInspector').inert=true;$('mapSummary').textContent='Recalculating… Showing the last completed map.';}mapPreviewTimer=setTimeout(()=>{if(mapChangeCount()&&!mapBusy())mapComputePreview(false);},350);}
...
function mapRender(){
 ...
 const replacedPlan=mapPlan!==p;
 if(replacedPlan){$('mapNodes').inert=false;mapOriginal=null;mapExcludedAreas.clear();mapPlan=p;mapDrafts=[];mapInspectDraft=null;mapExtraAreas.clear();mapProposedLinks=[];mapPreview=null;$('mapPreviewResult').hidden=true;}
 // Keep the last solved map on screen until a replacement is available. Draft
 // recipe expansion is useful for choice warnings, not for live result totals.
 if(mapGraph&&(!replacedPlan&&mapBusy()||mapOriginal&&!mapPreview?.preview_plan)){
  $('mapPending').textContent=mapChangeCount()?'Changes pending · recalculating…':'';
  return;
 }
 $('mapInspector').inert=false;
 $('mapUnusedMode').querySelectorAll('[data-unused-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.unusedMode===(state.map_unused_mode||'top'))));$('mapUnusedCount').value=state.map_unused_count??6;$('mapUnusedCount').hidden=(state.map_unused_mode||'top')!=='top';
 ...
 $('mapNodes').innerHTML=mapGraph.nodes.map(n=>{
  ...
  const shortages=n.crafts<1?craftMapShortages(shown,n.id):[];
  const detail=n.kind==='area'?(n.proposed?'Preview to calculate explores':mapCount(n.explores)+' explores'):n.kind==='inventory'?'Inventory and free perk supplies':n.free?'Unlimited':n.previewAim!=null?mapCount(n.crafts)+' / '+mapCount(n.previewAim)+' crafts':n.missing>=10?'':n.primary?mapCount(activeTargets()[n.id])+' wanted':n.draft?(solvedPreview?mapCount(n.crafts)+' crafts':mapDrafts.find(g=>g.item_id===n.id)?.consumer_mode==='available'?craftMapSurplusLabels(n).use+' · calculating':mapCount(n.draftCrafts)+' craft aim'):n.draftCrafts?mapCount(n.draftCrafts)+' more crafts needed':n.secondary&&n.crafts<1?(pausedBy.length?'0 crafts · blocked by '+pausedBy.map(id=>items[id].name).join(', '):shortages.length?'0 crafts · no spare '+shortages.map(r=>items[r.item_id]?.name||r.item_id).join(', '):'0 crafts · supplies needed'):n.crafts?mapCount(n.crafts)+' crafts · '+mapUnusedText(n):mapUnusedText(n);
...
function mapConnectionList(id){
 const connected=craftMapConnections(id,mapGraph.nodes,mapGraph.links);
 const group=(label,rows)=>rows.length?`<section><h4>${label} <small>${rows.length}</small></h4><div class="map-connection-grid">${rows.map(({node,quantity,phantom})=>{
  const area=node.kind==='area'?catalog.locations.find(a=>a.id===node.location):null;
  const icon=node.kind==='item'?itemIcon(node.id):area?.image?`<img class="gameicon" src="${esc(area.image.startsWith('/')?'https://farmrpg.com'+area.image:area.image)}" alt="">`:'<span class="map-source-icon" aria-hidden="true">⌖</span>';
  const count=node.free||node.kind==='inventory'&&mapGraph.nodes.find(n=>n.id===id)?.free?'Unlimited':quantity>0?mapCount(quantity)+(phantom?' proposed':''):'No current use';
  const description=node.name+' · '+count;
  return `<button class="map-connection" data-map-jump="${esc(node.id)}" aria-label="${esc(description)}" title="${esc(description)}">${icon}</button>`;
 }).join('')}</div></section>`:'';
 return `<div class="map-connections" aria-label="Connected nodes">${group('Inputs & sources',connected.inputs)}${group('Used by',connected.outputs)}${connected.inputs.length||connected.outputs.length?'':'<p class="hint">No connections yet.</p>'}</div>`;
}
function mapSelect(id,{preserveOptions=false}={}){
 if(mapGraph&&mapOriginal&&!mapPreview?.preview_plan){mapSelected=id;return;}
 ...
 if(mapSelected!==id)$('mapInspector').scrollTop=0;
 ...
 if(n.kind==='inventory'){$('mapInspector').innerHTML='<h3>Starting supplies</h3><p>These come from Starting inventory, passive production, or free perk supplies. Change their amounts in the existing input controls.</p>'+mapConnectionList(id);return;}
 ...
 const hasCraftConnection=craftMapConnections(id,mapGraph.nodes,mapGraph.links).outputs.some(r=>r.node.kind==='item');
 const surplusLabels=craftMapSurplusLabels(n);
 ...
 const shortages=n.crafts<1&&!pausedBy.length?craftMapShortages(mapPreview?.preview_plan||mapPlan,id).filter(r=>!missingInputs.includes(r.item_id)):[];
 $('mapInspector').innerHTML=`<h3>${itemName(id,n.name)}</h3>${opportunity?`<p class="map-tower-note">${opportunity.type} · Tower ${opportunity.level} · ${opportunity.level>(state.tower_level||0)?opportunity.level-(state.tower_level||0)+' levels ahead':'Earlier requirement'}</p>`:''}<p>${n.free?'Unlimited perk supply':n.crafts<1&&(n.secondary||n.draft)?'0 crafts':mapUnusedText(n)}${n.need?` · ${mapCount(Math.ceil(n.need))} needed by proposed crafts`:''}</p>${n.requiredUse>0?`<p class="map-required-use"><strong>${mapCount(n.requiredUse)} reserved for your goals.</strong>${n.voided?` The surplus is marked ${surplusLabels.sell}; optional crafts are paused.`:' Surplus choices do not change this amount.'}</p>`:''}${mapConnectionList(id)}${mapChangeCount()?'<p class="map-draft-note">Changes aren’t applied yet. Review them when you’re ready.</p>':''}${pausedBy.length?`<div class="map-gap-list">${pausedBy.map(ref=>`<button data-map-jump="${ref}">${itemName(ref,items[ref].name)} is set to ${state.map_node_usage[ref].mode==='void'?craftMapSurplusLabels(mapGraph.nodes.find(n=>n.id===ref)||{craftable:items[ref]?.craftable}).sell:'Leave Unused'} →</button>`).join('')}</div>`:''}${missingInputs.length?`<div class="map-gap-list"><p>Missing supplies</p>${missingInputs.map(ref=>`<button data-map-jump="${ref}">${itemName(ref,items[ref].name)} <small>${craftMapExternalSource(items[ref])?'Adjust supplies':'Choose a source'}</small> &rarr;</button>`).join('')}<p class="hint">Select an ingredient to adjust its supplies or choose an exploration source.</p></div>`:''}${shortages.length?`<div class="map-gap-list"><p>Not enough left for this craft</p>${shortages.map(r=>`<button class="map-shortage-link" data-map-jump="${esc(r.item_id)}"><span>${itemName(r.item_id,items[r.item_id].name)}<small>${r.primary?'Used by primary goals':'No spare supply'}</small></span><span aria-hidden="true">&rarr;</span></button>`).join('')}</div>`:''}${mapInspectDraft?`<div class="map-gap-list">${mapCraftGaps(mapInspectDraft.item_id,mapInspectDraft.cap).filter(([,q])=>q>=10).map(([ref,q])=>`<button class="map-gap" data-map-jump="${ref}">⚠ ${mapCount(Math.ceil(q))} ${esc(items[ref].name)} missing →</button>`).join('')}</div><p class="map-draft-note">Checking ${esc(items[mapInspectDraft.item_id].name)}; not added yet.</p><button data-map-add="${mapInspectDraft.item_id}" data-map-quantity="${mapInspectDraft.cap}" data-map-custom="${!!mapInspectDraft.automatic_supply}">Add this craft to draft</button>`:''}${draft&&draft.consumer_mode!=='available'?`<label>Required crafts <input id="mapDraftQuantity" type="number" min="0" max="100000000" step="1" value="${draft.cap}"></label>${draft.cap===0?'<p class="hint">Recipe added at 0 crafts. Set a quantity to see which supplies it needs.</p>':''}`:''}${applied&&!draft&&applied.consumer_mode!=='available'?`<label>Required crafts <input id="mapAppliedCap" type="number" min="0" max="100000000" step="1" placeholder="No cap" value="${applied.cap??''}"></label>${n.crafts<1?'<p class="map-gap">Not enough supplies to craft this yet. Dashed links show its ingredients.</p>':''}`:''}${items[id]?.craftable&&items[id]?.explorable&&!n.primary?'<div class="map-supply-choice"><p>This ingredient can be crafted or collected directly.</p><button id="mapExploreInstead">Explore for this ingredient</button><p class="hint">Choose a location to compare direct drops with crafting its ingredients.</p></div>':''}${externalSupply?`<div class="map-inventory-supply"><label for="mapExternalAmount">Supplied amount</label><input id="mapExternalAmount" type="number" min="0" max="1000000000000" step="1" value="${enteredSupply[id]??Math.ceil(n.automaticAmount||0)}"><button id="mapSetExternal" type="button">Set amount</button><button id="mapAutoExternal" type="button" ${n.autoSupply?'disabled':''}>Use required amount</button><small>${n.autoSupply?'Automatically provided from outside exploration. Set an amount to limit it.':'Your amount is a limit. Set zero to stop supplying this ingredient.'}</small></div>`:''}${!externalSupply&&inventoryNeed?`<div class="map-inventory-supply"><label for="mapInventoryAmount">Bring from inventory</label><input id="mapInventoryAmount" type="number" min="1" max="1000000000000" step="1" value="${inventoryNeed}"><button id="mapFromInventory" type="button">From inventory</button><small>${n.externalSource?'Not found by exploring. ':''}Adds this amount to starting supplies.</small></div>`:''}${n.missing>=10?`<div class="map-gap">⚠ ${mapCount(Math.ceil(n.missing))} ${esc(n.name)} missing</div><p class="hint">${items[id]?.explorable?'Choose an exploration source or bring supplies from inventory.':'Bring supplies from inventory to cover this gap.'}</p>`:''}${!n.free&&items[id]?.explorable?'<button id="mapFindSources" class="primary">Find exploration sources</button>':''}${mapHasCrafts(id)?'<button id="mapFindCrafts" class="primary">Choose a craft using this</button>':'<p class="hint">No further crafts use this item.</p>'}${n.explorationSupplied?'<p class="hint">Your route collects this item. Use Void/Sell if you don’t need the surplus.</p>':n.protected?'<p class="hint">Needed for your target. Change the target to remove it.</p>':`<button id="mapDeleteNode">Delete node</button>`}<div class="map-node-modes" aria-label="How to use this item"><button id="mapLeaveUnused" aria-pressed="${!n.useVoid&&!n.voided&&!['force','limit'].includes(state.map_node_usage?.[id]?.mode)}">${n.requiredUse>0?'Leave surplus unused':'Leave Unused'}</button><button id="mapRawUseVoid" ${hasCraftConnection?'':'disabled'} aria-pressed="${n.useVoid&&!n.voided}">Use + Void</button><button id="mapRawForce" aria-pressed="${state.map_node_usage?.[id]?.mode==='force'}" ${consumers.length?'':'disabled'}>Force Use All</button><button id="mapLimitToggle" aria-pressed="${state.map_node_usage?.[id]?.mode==='limit'}" ${consumers.length?'':'disabled'} aria-expanded="false" aria-controls="mapLimitControls">Limit uses</button><div id="mapLimitControls" hidden><label>Maximum ${esc(n.name)} to use<input id="mapRawLimit" type="number" min="0" max="1000000000000" value="${state.map_node_usage?.[id]?.amount??10000}"></label><button id="mapRawSetLimit">Apply limit</button></div><button id="mapVoidNode" aria-pressed="${!!n.voided}">${surplusLabels.sell}${n.requiredUse>0?' surplus':''}</button>${state.map_node_usage?.[id]?.mode==='force'?'<p class="hint">Prioritizes using this item. Current supplies and craft limits still apply; other crafts may get less.</p>':''}${consumers.length?'':hasCraftConnection?`<p class="hint">${surplusLabels.use} keeps your required crafts and marks the surplus for selling. Add a leftover craft to use more.</p>`:'<p class="hint">Connect a craft to enable the use options.</p>'}<p class="hint">Leave Unused and ${surplusLabels.sell} set connected optional crafts to zero. Primary goals stay protected.</p></div><div id="mapOptions"></div>`;
...
async function mapConsumers(id){
 ...
 try{const rows=await api('/api/consumers/'+id);if(token!==mapCandidateToken)return;
  ...
  const options=rows.filter(r=>r.direct&&(!chosen.has(r.id)||mapGraph.nodes.some(n=>n.id===r.id))).map(r=>{const existing=mapGraph.nodes.find(n=>n.id===r.id),added=chosen.has(r.id);const addOptions=added?{all:0,available:0}:mapCraftAddOptions(r.id,id);const gaps=added?[]:visibleCraftMapGaps(mapCraftGaps(r.id,addOptions.all));return {...r,existing,added,addOptions,gaps,complexity:craftMapRecipeComplexity(items,r.id,id,state),towerRank:rank(r)};}).sort((a,b)=>Number(!!b.existing)-Number(!!a.existing)||Number(a.gaps.length>0)-Number(b.gaps.length>0)||a.complexity-b.complexity||a.towerRank-b.towerRank||a.name.localeCompare(b.name));
 ...
 const existingNote=r.existing?`Already in map · ${mapCount(r.existing.crafts)} crafts`:'';
  const existingLink=r.existing?`<button class="map-existing-link" data-map-jump="${r.id}">Go to ${esc(r.name)} →</button>`:'';
  if(r.existing&&r.added)return `<div class="map-choice map-existing-choice"><span class="map-choice-title">${itemName(r.id,r.name)}</span><small>${existingNote}</small>${existingLink}</div>`;
 ...
 return `<details class="map-choice"><summary class="map-choice-summary"><span class="map-choice-title">${itemName(r.id,r.name)}</span><span class="map-choice-availability ${gaps.length?'partial':'ready'}">${esc(existingNote||availability)}</span><small>${r.direct?'Direct craft':'Through intermediates'}${badge&&badge.level>level?' · Tower '+badge.level+' ahead':''}</small></summary>${existingLink}${r.existing?'<p class="hint">This recipe already supplies other crafts. Its ingredients are connected.</p><details class="map-existing-extra"><summary>Make extra with leftovers</summary><p class="hint">These options add production to the same node. Required crafts stay protected.</p>':''}<div class="map-add-actions"><section class="map-add-option" aria-label="Use all option"><button ${quantity>0?'':'disabled'} data-map-add="${r.id}" data-map-quantity="${quantity}" data-map-focus="${id}">Force Use All · ${mapCount(quantity)} crafts</button><p class="hint">Use this material completely. Supply the gaps below.</p>${warningRows}</section><section class="map-add-option map-custom-option" data-map-custom-item="${r.id}"><label class="map-custom-label">Make up to <span class="map-custom-controls"><input type="number" min="1" max="100000000" step="1" value="10000" aria-label="Maximum crafts for ${esc(r.name)}"><button data-map-add="${r.id}" data-map-custom="true" data-map-quantity="10000">Add</button></span></label><div class="map-quantity-presets">${[1000,5000,10000,100000].map(n=>`<button data-map-preset="${n}">${n/1000}k</button>`).join('')}<span>crafts</span></div><p class="hint">Set a craft limit, then choose sources for any missing ingredients on the map.</p><div class="map-custom-gaps"></div></section><section class="map-add-option" aria-label="Use available option"><button data-map-add="${r.id}" data-map-quantity="${addOptions.available}" data-map-focus="${id}" data-map-available="true" >${craftMapSurplusLabels(mapGraph.nodes.find(n=>n.id===id)||{craftable:items[id]?.craftable}).use}</button><p class="hint">Share supplies from your current route. This does not add exploration.</p></section></div>${r.existing?'</details>':''}</details>`;}).join('')||'<p>No direct crafts match.</p>';}
...
async function mapSources(id){
 ...
 const token=++mapCandidateToken,host=$('mapOptions');const trigger=$('mapFindSources')||$('mapExploreInstead');if(trigger)trigger.after(host);host.textContent='Checking exploration sources…';
...
window.addEventListener('planner-calculating',()=>{$('mapNodes').inert=true;$('mapInspector').inert=true;if(mapGraph)$('mapPending').textContent='Recalculating… Showing the last completed map.';});
window.addEventListener('planner-cleared',()=>{mapJob++;mapDrafts=[];mapPositions={};mapSelected=null;mapRender();});
window.addEventListener('planner-settings-saved',()=>{if(!mapGraph&&state){mapPositions=JSON.parse(JSON.stringify(state.map_positions||{}));}});
