...
function mapQueuePreview(){clearTimeout(mapPreviewTimer);mapPreviewTimer=setTimeout(()=>{if(mapChangeCount()&&!mapBusy())mapComputePreview(false);},350);}
...
function mapChanged(){clearTimeout(mapPreviewTimer);mapJob++;if(mapPreviewRunning){browserPlanner.cancel();mapPreviewRunning=false;}mapPreview=null;$('mapPreviewResult').hidden=true;mapRender();}
function mapRender(){
 const p=mapCurrent();if(!p){craftMap.hidden=true;return;}craftMap.hidden=false;
 if(mapPlan!==p){mapOriginal=null;mapExcludedAreas.clear();mapPlan=p;mapDrafts=[];mapInspectDraft=null;mapExtraAreas.clear();mapProposedLinks=[];mapPreview=null;$('mapPreviewResult').hidden=true;}
 ...
 for(const node of mapGraph.nodes){const possible=mapPotential.graph.nodes.find(n=>n.id===node.id);const exact=exactPotential?.missing?.find(r=>r.item_id===node.id);node.potentialDemand=node.free?0:exactPotential?(exact?.quantity||0):Math.max(0,(possible?.need||0)-(node.need||0));node.potentialMissing=node.free?0:exactPotential?(exact?.quantity||0):Math.max(0,(possible?.missing||0)-(node.missing||0));}
...
function mapSelect(id,{preserveOptions=false}={}){
 ...
 $('mapInspector').innerHTML=`<h3>${itemName(id,n.name)}</h3>${opportunity?`<p class="map-tower-note">${opportunity.type} · Tower ${opportunity.level} · ${opportunity.level>(state.tower_level||0)?opportunity.level-(state.tower_level||0)+' levels ahead':'Earlier requirement'}</p>`:''}<p>${n.free?'Unlimited perk supply':mapUnusedText(n)}${n.need?` · ${mapCount(Math.ceil(n.need))} needed by proposed crafts`:''}</p>${mapChangeCount()?'<p class="map-draft-note">Changes aren’t applied yet. Review them when you’re ready.</p>':''}${pausedBy.length?`<div class="map-gap-list">${pausedBy.map(ref=>`<button data-map-jump="${ref}">${itemName(ref,items[ref].name)} is set to ${state.map_node_usage[ref].mode==='void'?'Void/Sell':'Leave Unused'} →</button>`).join('')}</div>`:''}${mapInspectDraft?`<div class="map-gap-list">${mapCraftGaps(mapInspectDraft.item_id,mapInspectDraft.cap).filter(([,q])=>q>=10).map(([ref,q])=>`<button class="map-gap" data-map-jump="${ref}">⚠ ${mapCount(Math.ceil(q))} ${esc(items[ref].name)} missing →</button>`).join('')}</div><p class="map-draft-note">Checking ${esc(items[mapInspectDraft.item_id].name)}; not added yet.</p><button data-map-add="${mapInspectDraft.item_id}" data-map-quantity="${mapInspectDraft.cap}" data-map-custom="${!!mapInspectDraft.automatic_supply}">Add this craft to draft</button>`:''}${draft&&draft.consumer_mode!=='available'?`<label>Required crafts <input id="mapDraftQuantity" type="number" min="0" max="100000000" step="1" value="${draft.cap}"></label>${draft.cap===0?'<p class="hint">Recipe added at 0 crafts. Set a quantity to see which supplies it needs.</p>':''}`:''}${applied&&!draft&&applied.consumer_mode!=='available'?`<label>Required crafts <input id="mapAppliedCap" type="number" min="0" max="100000000" step="1" placeholder="No cap" value="${applied.cap??''}"></label>${n.crafts<1?'<p class="map-gap">Not enough supplies to craft this yet. Dashed links show its ingredients.</p>':''}`:''}${n.potentialDemand>=10?`<div class="map-potential"><p>Potential demand: <strong>${mapCount(Math.ceil(n.potentialDemand))}</strong></p><button id="mapMeetPotential">Meet potential demand</button><p class="hint">Set a fixed goal from current supplies, then choose where to get missing ingredients.</p></div>`:''}${items[id]?.craftable&&items[id]?.explorable&&!n.primary?'<div class="map-supply-choice"><p>This ingredient can be crafted or collected directly.</p><button id="mapExploreInstead">Explore for this ingredient</button><p class="hint">Choose a location to compare direct drops with crafting its ingredients.</p></div>':''}${n.missing>0||n.draftCrafts&&!n.draft?`<div class="map-inventory-supply"><label for="mapInventoryAmount">Bring from inventory</label><input id="mapInventoryAmount" type="number" min="1" max="1000000000000" step="1" value="${Math.ceil(n.missing||n.draftCrafts*items[id].output_quantity)}"><button id="mapFromInventory" type="button">From inventory</button><small>Adds this amount to starting supplies.</small></div>`:''}${n.missing>=10?`<div class="map-gap">⚠ ${mapCount(Math.ceil(n.missing))} ${esc(n.name)} missing</div><p class="hint">Choose an exploration source or add what you already have in Settings.</p>`:''}${!n.free&&consumers.length?`<div class="map-demand-share"><label>Meet potential demand <input id="mapDemandPercent" type="number" min="0" max="100" step="1" value="${consumers.find(g=>g.demand_group===id)?.demand_percent??100}">%</label><button id="mapSetDemand">Share across connected crafts</button><p class="hint">Use + Void crafts share proportionally. Forced quantities and limits keep their own aims.</p></div>`:''}${!n.free&&(n.missing>0||items[id]?.explorable)?'<button id="mapFindSources" class="primary">Find exploration sources</button>':''}${mapHasCrafts(id)?'<button id="mapFindCrafts" class="primary">Choose a craft using this</button>':'<p class="hint">No further crafts use this item.</p>'}${n.explorationSupplied?'<p class="hint">Your route collects this item. Use Void/Sell if you don’t need the surplus.</p>':n.protected?'<p class="hint">Needed for your target. Change the target to remove it.</p>':`<button id="mapDeleteNode">Delete node</button>`}<div class="map-node-modes" aria-label="How to use this item"><button id="mapLeaveUnused" aria-pressed="${!n.useVoid&&!n.voided&&!['force','limit'].includes(state.map_node_usage?.[id]?.mode)}">Leave Unused</button><button id="mapRawUseVoid" ${consumers.length?'':'disabled'} aria-pressed="${n.useVoid&&!n.voided}">Use + Void</button><button id="mapRawForce" aria-pressed="${state.map_node_usage?.[id]?.mode==='force'}" ${consumers.length?'':'disabled'}>Force Use All</button><button id="mapLimitToggle" aria-pressed="${state.map_node_usage?.[id]?.mode==='limit'}" ${consumers.length?'':'disabled'} aria-expanded="false" aria-controls="mapLimitControls">Limit uses</button><div id="mapLimitControls" hidden><label>Maximum ${esc(n.name)} to use<input id="mapRawLimit" type="number" min="0" max="1000000000000" value="${state.map_node_usage?.[id]?.amount??10000}"></label><button id="mapRawSetLimit">Apply limit</button></div><button id="mapVoidNode" aria-pressed="${!!n.voided}">Void/Sell</button>${consumers.length?'':'<p class="hint">Connect a craft to enable Use + Void, Force Use All, and Limit uses.</p>'}<p class="hint">Leave Unused and Void/Sell set connected optional crafts to zero. Primary goals stay protected.</p></div><div id="mapOptions"></div>`;
 function updateConsumer(goal,cap,available=false){
  state.secondary=state.secondary.filter(g=>g.item_id!==goal.item_id);
  let g=mapDrafts.find(g=>g.item_id===goal.item_id);
  if(!g){g={...goal};mapDrafts.push(g);}
  Object.assign(g,{cap:available?null:cap,user_cap:false,consumer_mode:available?'available':'fixed',demand_group:null,available_only:available,void_source:available?id:null,allow_exploration:false,automatic_supply:false,automatic_batch:true});
 }
 ...
 function setRawUsage(force){
  ...
  const factor=1/(1+(state.resource_saver||0)/100);
  function usage(ref,q){if(ref===id)return q;return Object.entries(items[ref]?.direct_ingredients||{}).reduce((sum,[child,n])=>sum+usage(child,q*n*factor/(items[ref].output_quantity||1)),0);}
  const options=consumers.map(goal=>{const rate=Object.entries(items[goal.item_id].direct_ingredients).reduce((sum,[ref,q])=>sum+usage(ref,q*factor),0);const aim=mapCraftAddOptions(goal.item_id,id).all;return {goal,rate,aim};}).filter(x=>x.rate>0&&x.aim>0);
  const demand=options.reduce((sum,x)=>sum+x.rate*x.aim,0);if(!demand)return;
  const supply=Math.max(0,n.stock)+consumers.reduce((sum,g)=>{const x=options.find(x=>x.goal.item_id===g.item_id);return sum+(x?x.rate*(g.cap||0):0);},0);
  const fraction=Math.min(1,(force?supply:amount)/demand);
  mapBeginEdit();state.map_node_usage??={};state.map_node_usage[id]={mode:force?'force':'limit',amount:force?null:amount};
  for(const {goal,aim} of options){updateConsumer(goal,Math.floor(aim*fraction+1e-8));if(!force){const g=mapDrafts.find(g=>g.item_id===goal.item_id);g.consumer_mode='available';g.user_cap=true;}}
...
async function mapSources(id){
 ...
 const sourceNode=mapGraph.nodes.find(n=>n.id===id);const need=sourceNode?.missing||sourceNode?.potentialMissing||0;
 host.innerHTML='<h4>Exploration sources</h4>'+selectedSources.map(area=>`<button data-map-remove-source="${esc(area)}" data-map-source-item="${id}">Remove ${esc(catalog.locations.find(a=>a.id===area)?.name||area)} source</button>`).join('')+rows.map(r=>{
  ...
  return `<button class="map-candidate" data-map-area="${esc(r.location_id)}" data-map-source-item="${id}"><strong>${esc(r.location_name||r.location_id)}</strong><small>${fmt(rate)} items per explore · ${mapEstimateInputs().plan.areas.some(a=>a.location_id===r.location_id)?'Already in route':selectedSources.includes(r.location_id)?'Selected source':'Add this location'}</small>${coverage.map(({node,supplied})=>`<small class="map-source-coverage">${itemName(node.id,node.name)} · ${!node.missing?'Also supplies this connected ingredient':supplied>=node.missing?'Covers this shortage':`Supplies about ${Math.min(100,Math.floor(100*supplied/node.missing))}% of shortage`}</small>`).join('')}${gaps.length&&!coverage.length?'<small>No other missing ingredients supplied'+(otherSources.some(s=>s.unavailable)?' by the sources checked':'')+'.</small>':''}</button>`;
...
function mapUnusedText(n){const total=Math.max(n.stock,n.totalSupply||0);return total>0&&n.stock>=1&&n.stock<total?`${mapCount(n.stock)} out of ${mapCount(total)} unused (${Math.floor(100*n.stock/total)}%)`:`${mapCount(n.stock)} unused`;}
function mapNext(){
 if(!mapGraph)return;
...
$('mapInspector').onclick=async e=>{
 ...
 const id=removeSource.dataset.mapSourceItem,area=removeSource.dataset.mapRemoveSource;mapBeginEdit();state.map_planning=true;state.map_sources[id]=(state.map_sources[id]||[]).filter(a=>a!==area);
 ...
 if(add){
  ...
  if(mapGoals().length>=20){$('mapOptions').textContent='The plan supports 20 leftover crafts. Remove one to continue.';return;}
 ...
 if(area){
  const sourceId=area.dataset.mapSourceItem;mapBeginEdit();state.map_planning=true;state.map_sources??={};state.map_sources[sourceId]=[...new Set([...(state.map_sources[sourceId]||[]),area.dataset.mapArea])];
...
async function mapComputePreview(showReview=false){
 ...
 }catch(e){if(token===mapJob){const message=e.message.split('\n').filter(Boolean).at(-1);$('mapSummary').textContent=message;$('mapReviewStatus').textContent=message;}}finally{if(token===mapJob){mapPreviewRunning=false;$('mapPreview').disabled=!mapChangeCount();$('mapReviewTop').disabled=!mapChangeCount();}}
};
