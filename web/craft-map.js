...
function mapConnectionList(id){
 ...
 const group=(label,rows)=>rows.length?`<section><h4>${label} <small>${rows.length}</small></h4>${rows.map(({node,quantity,phantom})=>{
  const name=node.kind==='item'?itemName(node.id,node.name):esc(node.name);
  ...
  return `<button class="map-connection" data-map-jump="${esc(node.id)}"><span>${name}<small>${count}</small></span><span aria-hidden="true">→</span></button>`;
 }).join('')}</section>`:'';
